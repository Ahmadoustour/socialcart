...
export const SocialFeed: React.FC<SocialFeedProps> = ({
  ...
  // Instant optimistic like cache for 0ms zero-latency user experience
  const [optimisticLikes, setOptimisticLikes] = useState<Record<string, { isLiked: boolean; count: number }>>({});

  const handleInstantLike = (postId: string) => {
    ...
    const baseIsLiked = !isGuest && isPostLikedByUser(post, currentUser);
    const baseCount = typeof post.likesCount === 'number' ? post.likesCount : 0;

    const currentEntry = optimisticLikes[postId];
    const currentIsLiked = currentEntry !== undefined ? currentEntry.isLiked : baseIsLiked;
    const currentCount = currentEntry !== undefined ? currentEntry.count : baseCount;

    const nextIsLiked = !currentIsLiked;
    const nextCount = Math.max(0, currentCount + (nextIsLiked ? 1 : -1));

    // Zero-millisecond instantaneous visual response!
    setOptimisticLikes(prev => ({
      ...prev,
      [postId]: { isLiked: nextIsLiked, count: nextCount }
    }));

    // Trigger state & server synchronization in background
    onLikePost(postId);
  };
    ...
    <div className="w-full max-w-7xl mx-auto pb-12 animate-fadeIn overflow-x-hidden">
      ...
      <div className="space-y-6">
        {filteredPosts.length === 0 ? (
          ...
          const isLiked = isGuest ? false : (opt !== undefined ? opt.isLiked : baseIsLiked);
          const displayLikesCount = opt !== undefined ? opt.count : baseCount;
            ...
            >
              ...
              <div className="px-4 py-2.5 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between text-xs text-slate-500 dark:text-slate-400">
                <span className="font-semibold flex items-center gap-1.5">
                  <span className={`inline-block transition-transform duration-150 ${isLiked ? 'scale-110' : ''}`}>❤️</span>
                  <span>{displayLikesCount} إعجاب</span>
                </span>
              ...
              <div className="px-2 sm:px-4 py-2 border-t border-slate-100 dark:border-slate-800 grid grid-cols-3 gap-1 sm:gap-2">
                <button
                  type="button"
                  onClick={() => handleInstantLike(post.id)}
                  className={`py-2 rounded-xl text-[11px] sm:text-xs font-bold flex items-center justify-center gap-1 sm:gap-1.5 transition cursor-pointer select-none active:scale-95 ${
                    isLiked
                      ? 'bg-rose-50 dark:bg-rose-950/40 text-rose-600 dark:text-rose-400'
                      : 'hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-600 dark:text-slate-300'
                  }`}
                >
                  <Heart className={`w-3.5 h-3.5 sm:w-4 sm:h-4 transition-transform duration-150 ${isLiked ? 'fill-rose-500 text-rose-500 scale-110' : ''}`} />
